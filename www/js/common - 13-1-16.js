	...
	function createAddnewcontact(first_name,last_name,email,mobile,photos) {
		
		alert('add');
		
		var options = new ContactFindOptions();
		options.filter="A new Girishas";
		...
		function onSuccess(contacts) {
			...
			if(contacts.length>0){
				// countget  = contacts.length-1;
				// full_name = contacts[countget].name.givenName+contacts[countget].name.familyName;
				// full_name = full_name.slice(-1);
			  	// alert(full_name); 
		        confirmcheak = confirm('Alredy added contact');
			}
			...
			if(contacts.length==0 || confirmcheak){ 
				...
				contact.name = {givenName: 'A new', familyName: 'Girishas'};
				contact.displayName = "A new Girishas";

				var phoneNumbers = [];
				phoneNumbers[0] = new ContactField('mobile', '917-555-5434', true); // preferred number
		    	contact.phoneNumbers = phoneNumbers;

				var emails = [];
				emails[0] = new ContactField('work', 'dev.girishas@gmail.com', true); // preferred email
		    	contact.emails = emails;

				var photos = [];
				photos[0] = new ContactField('photos','https://www.nd2nosmart.cards/nd2no/img/front_images/logo.png',true); // preferred profile picture
