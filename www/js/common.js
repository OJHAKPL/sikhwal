	...
	function homeLogin() {
		...
		$.ajax({
			type: 'POST',
			url: 'http://192.168.1.5/nd2no/admin/web-login',
			beforeSend: function(){
				$('.loader_login').show();
			},
			complete: function(){
	...
	function showFoldercards(folderId,folderName) {
		...
		$.ajax({
			type: 'POST',
			url: 'http://192.168.1.5/nd2no/admin/web-folder-cards',
			beforeSend: function(){
				$('.loader_foldercardlist').show();
			},
			complete: function(){
				$('.loader_foldercardlist').hide();
			},
	...
	function cartDetailsNotification(cardId) {
		...
		$.ajax({
			type: 'POST',
			url: 'http://192.168.1.5/nd2no/admin/web-cards-detail',
			beforeSend: function(){
			...
			success: function(cardDetails){
				...
				$.each(cardDetailsArr, function(i, firstRow) {	
					...
					shareUrl = "http://192.168.1.5/nd2no/card/"+firstRow.card.post_key+"-"+cardId+"/mobile";
		...
		if(user_id){		
			...
			$.ajax({
				type: 'POST',
				url: 'http://192.168.1.5/nd2no/admin/web-reset-notification',
	...
	function sharedcardlist(){
		...
		if(user_id){
			...
			$.ajax({
				type: 'POST',
				url: 'http://192.168.1.5/nd2no/admin/web-shared-cards',
				beforeSend: function(){
	...
	function removeSelected(cardId) {
		...
		$.ajax({
			type: 'POST',
			url: 'http://192.168.1.5/nd2no/admin/web-selected-cards',
			beforeSend: function(){
				$('.loader_cardselectlist').show();
			},
			complete: function(){
				$('.loader_cardselectlist').hide();
			},
			data: { "selected_card_ids": items },
			success: function(cardlist){
				...
				if(!cardlistArr.error) {
					...
					$.each( cardlistArr, function(i, row1) {
						...
						if(card_ids){
							...
							shareUrl = 'http://192.168.1.5/nd2no/mobi-shared/'+card_ids;
						}
						if(cardImages2){
	...
	function editCard(cardId){
		...
		$.ajax({
			type: 'POST',
			url: 'http://192.168.1.5/nd2no/admin/web-update-card/'+cardId+'',
			beforeSend: function(){
		...
		$('#edit_card').validate({
			...
			submitHandler:function (form) {
				...
				*/
				...
				$.post(
					"http://192.168.1.5/nd2no/admin/web-update-card/"+card_id+"",{
	...
	function viewProfile(){
		...
		if(user_id){
			...
			$.ajax({
				type: 'POST',
				url: 'http://192.168.1.5/nd2no/admin/web-user-info',
				beforeSend: function(){
					$('.loader_userupdate').show();
				},
				complete: function(){
					$('.loader_userupdate').hide();
				},
				data: { "id": user_id },
				success: function(profiledetails){
	...
	function photoUpload(userId) {
	    ...
	    ft = new FileTransfer();
	    ft.upload(sPicData, "http://192.168.1.5/nd2no/admin/user-photo/"+userId, win, fail, options);
	...
	function EditProfileSubmit(){
		...
		$('#editprofile').validate({
			...
			submitHandler:function (form) {
				...
				$.ajax({
					type: 'POST',
					url: 'http://192.168.1.5/nd2no/admin/web-update-profile',
					beforeSend: function(){
						$('.loader_useredit').show();
					},
					complete: function(){
	...
	function cardlist() {
		...
		$.mobile.changePage("#card-list-loader",{allowSamePageTransition:false,reloadPage:false,changeHash:true,transition:"slide"});
		...
		if(user_id){
			...
			$.ajax({
				type: 'POST',
				url: 'http://192.168.1.5/nd2no/admin/web-user-cards',
				beforeSend: function(){
	...
	function moveTocard(){
		...
		if(user_id){	
			...
			$.ajax({
				type: 'POST',
				url: 'http://192.168.1.5/nd2no/admin/web-folder-list',
				beforeSend: function(){
	...
	function myfolderList(){
		...
		if(user_id){
			...
			if(localStorage.getItem('email')) {
				...
				$.ajax({
					type: 'POST',
					url: 'http://192.168.1.5/nd2no/admin/web-show-folders',
					beforeSend: function(){
	...
	function cardLink(cardId){ 
		...
		$.ajax({
			type: 'POST',
			url: 'http://192.168.1.5/nd2no/admin/web-link-card',
			beforeSend: function(){
			...
			success: function(cardlink){
				...
				var profileArr = jQuery.parseJSON(cardlink);
				if(!profileArr.error) { 
	...
	function editscroller(cardId){ 
		...
		$.ajax({
			type: 'POST',
			url: 'http://192.168.1.5/nd2no/admin/web-scroller-card',
			beforeSend: function(){
			...
			data: { "card_id": cardId },
			success: function(cardscroller){
				
				$('.editscrollerremove').remove();
				
				var cardscrollerArr = jQuery.parseJSON(cardscroller);
				if(!cardscrollerArr.error) { 
					
					$.each( cardscrollerArr, function(i, row) {
						scrollChaek = '';
	...
	function notificationList() {
		...
		if(user_id){
			...
			$.ajax({
				type: 'POST',
				url: 'http://192.168.1.5/nd2no/admin/web-notification-list',
				beforeSend: function(){
					$('.loader_notifactionlist').show();
				},
				complete: function(){
					$('.loader_notifactionlist').hide();
				},
				data: { "user_id": user_id },
				success: function(notifylist){
					
					var notifylistArr = jQuery.parseJSON(notifylist);
					if(!notifylistArr.error) {
						...
						$(".errorMsgShow").show();
						$(".errorMsgShow").addClass("error");
						$(".errorMsgShow").text(notifylistArr.error);
						setTimeout(function() {
							$('.errorMsgShow').hide();
						}, 4000);
					}
	...
	function forgotPassword(){
		...
		$.ajax({
			type: 'POST',
			url: 'http://192.168.1.5/nd2no/admin/web-forget-password',
			beforeSend: function(){
	...
	$(document).ready(function(){
		...
		$('#register_form').validate({
			...
			submitHandler:function (form) {	
				...
				$.ajax({
					type: 'POST',
					url: 'http://192.168.1.5/nd2no/admin/web-register',
					beforeSend: function(){
		...
		$('#changepassword_form').validate({
			...
			submitHandler:function (form) {	
				...
				if(user_id){
					...
					$.ajax({
						type: 'POST',
						url: 'http://192.168.1.5/nd2no/admin/web-cahnge-password',
						beforeSend: function(){
